...
const FAQAccordion = () => {
  ...
  return (
    <div className="max-w-[1072px] absolute top-[-120px] bg-[#FFF] rounded-[20px] mx-[179px] py-[16px] px-[40px] max-900:px-[20px]  max-1250:mx-[70px] max-900:top-[-90px]">
      {faqs.map((faq, index) => (
        <>
          <div
            key={index}
          ...
          >
            <button
              className="py-[30px] px-[40px] flex justify-between items-center w-full text-left text-[#100F0F] focus:outline-none max-900:px-[0px] max-900:py-[12px]"
              onClick={() => toggleAccordion(index)}
            >
              <p className="break-all leading-[26px]">{faq.question}</p>
          ...
          {activeIndex === index && (
            <div className="text-[#5C5C5C] py-[20px] px-[40px] bg-[#F5F5F5] min-h-[86px] flex items-center transition-all duration-500 overflow-hidden max-w-[full] leading-[26px] max-900:px-[5px] max-900:py-[12px]">
              <p className="break-all">{faq.answer}</p>
            </div>
          )}
        </>
      ))}
